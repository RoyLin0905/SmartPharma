            ...
            adminAlertBox.innerHTML = `
                ...
                <div>
                    ...
                    <p style="margin: 0; font-size: 0.92rem; color: var(--text-dark); line-height: 1.6;">
                        ...
                        <span class="badge badge-danger" style="font-size:0.75rem; margin-top:4px;">[AI 決策執行]</span> 系統已自動將德怡藥局與衛生所之<b>安全備貨天數由 7 天調升至 10.5 天 (1.5x)</b>，防範因雨道路受阻中斷。
                    </p>
                </div>
            `;
        } else if (mode === 'typhoon') {
...
const STATIONS_METADATA = {
    ...
    'FUXING_HC': { name: '復興區衛生所', address: '復興區澤仁里中正路25號', phone: '(03) 382-2325', hours: '08:00-17:00 (週末休診)', lat: 24.8213, lng: 121.3524, district: '復興區' },
    'KAO_CLINIC': { name: '高揚威家醫科診所', address: '復興區澤仁里忠孝路32號', phone: '(03) 382-1688', hours: '08:00-17:00 (週末休診)', lat: 24.8209, lng: 121.3525, district: '復興區' },
...
function doLogin() {
    ...
    else if (user === 'fuxing_chen') loginAs('pharmacist', 'FUXING_HC', '復興區衛生所');
    else if (user === 'fuxing_kao') loginAs('pharmacist', 'KAO_CLINIC', '高揚威診所');
    ...
    try {
        ...
        if (localInv) {
            ...
            dbInventory = [
                ...
                { drugCode: 'B023245100', drugChineseName: '普拿疼止痛錠 (Panadol) - 退燒止痛', drugEnglishName: 'Panadol 500mg', price: 15, rxOnly: false, atcCode: 'N02BE01', dosageForm: '錠劑', singleCompoundFlag: '單方', manufacturer: 'GSK', temperatureReq: 'Room', expiryDays: 240, batchNo: 'B22938', stock_DEYI: 40, stock_FUXING_HC: 30, stock_KAO_CLINIC: 15, stock_SHISHENG_FX: 100, stock_GREAT_TREE: 120, stock_SHISHENG_KZ: 80, stock_ZISHENG: 60 },
                { drugCode: 'B023245199', drugChineseName: '布洛芬止痛膠囊 (Ibuprofen) - 相似替代藥', drugEnglishName: 'Ibuprofen 400mg', price: 20, rxOnly: false, atcCode: 'N02BE99', dosageForm: '膠囊劑', singleCompoundFlag: '單方', manufacturer: 'AstraZeneca', temperatureReq: 'Room', expiryDays: 190, batchNo: 'B33842', stock_DEYI: 25, stock_FUXING_HC: 20, stock_KAO_CLINIC: 10, stock_SHISHENG_FX: 90, stock_GREAT_TREE: 100, stock_SHISHENG_KZ: 70, stock_ZISHENG: 50 },
                { drugCode: 'I012345678', drugChineseName: '胰島素注射劑 (Insulin) - 糖尿病慢箋', drugEnglishName: 'Novomix 30 Flexpen', price: 800, rxOnly: true, atcCode: 'A10AD05', dosageForm: '注射劑', singleCompoundFlag: '複方', manufacturer: 'Novo Nordisk', temperatureReq: 'Cold Chain 2-8°C', expiryDays: 25, batchNo: 'B90234', stock_DEYI: 3, stock_FUXING_HC: 2, stock_KAO_CLINIC: 1, stock_SHISHENG_FX: 45, stock_GREAT_TREE: 60, stock_SHISHENG_KZ: 30, stock_ZISHENG: 25 },
                { drugCode: 'I012345699', drugChineseName: '美獲平糖適錠 (Metformin) - 糖尿病相似替代藥', drugEnglishName: 'Metformin 500mg', price: 10, rxOnly: true, atcCode: 'A10BA02', dosageForm: '錠劑', singleCompoundFlag: '單方', manufacturer: 'Sandoz', temperatureReq: 'Room', expiryDays: 160, batchNo: 'B78129', stock_DEYI: 8, stock_FUXING_HC: 10, stock_KAO_CLINIC: 5, stock_SHISHENG_FX: 100, stock_GREAT_TREE: 80, stock_SHISHENG_KZ: 60, stock_ZISHENG: 40 },
                { drugCode: 'E060800100', drugChineseName: '倍拉維 (Paxlovid) - COVID-19專用藥', drugEnglishName: 'Paxlovid Film-Coated', price: 20000, rxOnly: true, atcCode: 'J05AE30', dosageForm: '包衣錠劑', singleCompoundFlag: '複方', manufacturer: 'Pfizer', temperatureReq: 'Room', expiryDays: 90, batchNo: 'B11234', stock_DEYI: 5, stock_FUXING_HC: 8, stock_KAO_CLINIC: 2, stock_SHISHENG_FX: 20, stock_GREAT_TREE: 30, stock_SHISHENG_KZ: 15, stock_ZISHENG: 10 }
            ];
            localStorage.setItem('SmartPharma_Inventory', JSON.stringify(dbInventory));
        }
    }
...
function updateSystemState() {
    ...
    if (currentRole === 'pharmacist') renderExpiringTable();
...
function renderInventoryTable() {
    ...
    if (currentRole === 'pharmacist') {
        let name = STATIONS_METADATA[currentStation] ? STATIONS_METADATA[currentStation].name : '本店';
        thHtml += `<th>${name} 庫存</th>`;
    } else if (currentRole === 'buyer') {
        thHtml += `<th>德怡藥局(復興)</th><th>衛生所(復興)</th><th>高揚威診所(復興)</th>`;
    } else {
        // admin or driver
        thHtml += `<th>德怡(復興)</th><th>衛生所(復興)</th><th>高揚威(復興)</th><th>新資生復興(大溪)</th><th>大樹康莊(大溪)</th><th>新資生康莊(大溪)</th><th>資生(大溪)</th>`;
    }
    ...
    dbInventory.filter(item => {
        ...
        if (currentRole === 'buyer') {
            actionBtn = `
                <div style="display:flex; gap:5px; flex-direction:column;">
                    ...
                    <button class="btn btn-primary" style="font-size:0.75rem;padding:4px 8px; background:var(--secondary-color);" onclick="triggerReservationFlow('${item.drugChineseName}', 'FUXING_HC', ${item.rxOnly})">預約衛生所</button>
                    <button class="btn btn-success" style="font-size:0.75rem;padding:4px 8px; background:#8b5cf6;" onclick="triggerReservationFlow('${item.drugChineseName}', 'KAO_CLINIC', ${item.rxOnly})">預約高揚威</button>
                </div>
            `;
        ...
        if (currentRole === 'pharmacist') {
            let myStock = item['stock_' + currentStation] || 0;
            stockCells = `<td style="${myStock < threshold ? 'color:var(--danger-color);font-weight:bold;' : ''}">${myStock} 盒</td>`;
        } else if (currentRole === 'buyer') {
            stockCells = `
                <td style="${item.stock_DEYI < threshold ? 'color:var(--danger-color);font-weight:bold;' : ''}">${item.stock_DEYI} 盒</td>
                <td style="${item.stock_FUXING_HC < threshold ? 'color:var(--danger-color);font-weight:bold;' : ''}">${item.stock_FUXING_HC} 盒</td>
                <td style="${item.stock_KAO_CLINIC < threshold ? 'color:var(--danger-color);font-weight:bold;' : ''}">${item.stock_KAO_CLINIC} 盒</td>
            `;
        } else {
            stockCells = `
                <td style="${item.stock_DEYI < threshold ? 'color:var(--danger-color);font-weight:bold;' : ''}">${item.stock_DEYI} 盒</td>
...
function renderExpiringTable() {
    ...
    let myStockField = 'stock_' + currentStation;
    let expiringItems = dbInventory.filter(item => item.expiryDays < 45 && (item[myStockField] || 0) > 0);
    
    if (document.getElementById('dash-expiry-count')) {
        document.getElementById('dash-expiry-count').innerText = expiringItems.length;
    }
    
    if (expiringItems.length === 0) { 
        tbody.innerHTML = `<tr><td colspan="6" style="text-align:center; color:var(--text-muted);">目前無 45 天內即期藥品。</td></tr>`; 
        return; 
    }
    
    expiringItems.forEach(item => {
        let actionBtn = `<button class="btn btn-primary" style="padding:6px 12px;font-size:0.8rem; background:var(--warning-color);" onclick="openTransferModal('${item.drugChineseName}')"><i class="fa-solid fa-bullhorn"></i> 主動對外調發</button>`;
        let myStock = item[myStockField] || 0;
        
        const tr = document.createElement('tr');
        tr.innerHTML = `
            <td><code>${item.batchNo}</code></td>
            <td><strong>${item.drugChineseName}</strong></td>
            <td style="color:var(--danger-color); font-weight:bold;">${item.expiryDate}</td>
            <td><span class="badge badge-danger">剩 ${item.expiryDays} 天</span></td>
            <td>${myStock} 盒</td>
            <td>${actionBtn}</td>
        `;
        tbody.appendChild(tr);
...
function renderAdminCharts() {
    const ctxScrap = document.getElementById('scrapRateChart'); 
    const ctxSeasonal = document.getElementById('seasonalChart');
    if (!ctxScrap || !ctxSeasonal) return;
    
    if (scrapChartInstance) scrapChartInstance.destroy();
    if (seasonalChartInstance) seasonalChartInstance.destroy();

    // Goal 1: Quantify waste (Comparing last year vs this year)
    scrapChartInstance = new Chart(ctxScrap, {
        type: 'bar',
        data: {
            labels: ['1月', '2月', '3月', '4月', '5月', '6月'],
            datasets: [
                { label: '去年同期報廢率(%)', data: [11.2, 10.8, 10.1, 9.4, 9.1, 8.8], backgroundColor: '#cbd5e1', borderRadius: 4 },
                { label: '今年同期報廢率(%) (跨區調撥後)', data: [4.9, 4.5, 4.2, 3.8, 3.5, 3.1], backgroundColor: '#0d9488', borderRadius: 4 }
            ]
        },
        options: {
            responsive: true,
